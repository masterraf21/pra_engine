...
class ClockSkew:
    def __init__(self, endpoint, skew) -> None:
...
def adjust_timestamps(span, skew):
...
def adjust(node: SpanNode, skew_from_parent: ClockSkew):
    if skew_from_parent:
...
def tree_corrected_for_clock_skew(spans: list[dict], debug=False):
    if len(spans) == 0:
        return SpanNode()
